...
const StatCard: React.FC<{ icon: any, label: string, value: string | number, color: string }> = ({ icon: Icon, label, value, color }) => (
  <div className={cn("bg-opacity-10 p-4 rounded-2xl border flex items-start gap-3", color.replace('text-', 'bg-').replace('600', '50'), color.replace('text-', 'border-'))}>
    ...
    <div>
      ...
      <h3 dir="ltr" className={cn("text-lg font-bold font-mono leading-none tracking-tight", color)}>{value}</h3>
    </div>
  </div>
);
    ...
    <div className="sticky top-[8.5rem] bg-gray-50 dark:bg-slate-950 w-full shadow-lg overflow-hidden flex flex-col border dark:border-slate-800 animate-in fade-in duration-300 max-h-[calc(100vh-10rem)] rounded-[2rem]">
      ...
      <div className="flex justify-between items-center p-4 border-b dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="overflow-hidden">
            <h2 className="text-base font-bold text-gray-800 dark:text-slate-100 leading-tight truncate">{product.name}</h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <StatCard icon={TrendingUp} label="سعر البيع" value={formatCurrency(product.sale_price ?? product.selling_price ?? 0)} color="text-emerald-600" />
        </div>

        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border dark:border-slate-800">
          <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-2">بيانات أساسية</h4>
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard icon={Package} label="إجمالي المشتريات" value={formatNumberDisplay(product.total_purchases_qty || 0)} color="text-blue-600" />
          <StatCard icon={TrendingDown} label="إجمالي المبيعات" value={formatNumberDisplay(product.total_sales_qty || 0)} color="text-blue-600" />
        </div>

        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border dark:border-slate-800">
          <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-3 flex items-center gap-1.5"><Layers size={12} /> توزيع المخزون</h4>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border dark:border-slate-800">
          <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-3 flex items-center gap-1.5"><Car size={12} /> التوافق</h4>
          <div className="space-y-2">
