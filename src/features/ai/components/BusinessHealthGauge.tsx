        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-gray-800 dark:text-white">مؤشر صحة الأعمال</h3>
                ...
                <button
                    onClick={analyze}
                    disabled={isLoading}
                    className="px-3 py-1.5 text-[10px] font-bold bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-300 text-white rounded-xl transition-all"
                >
                    ...
                    <div className="flex items-center justify-center">
                        <div className="relative w-32 h-32">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                <span className="text-3xl font-bold font-mono" style={{ color: scoreColor(result.score) }}>{result.score}</span>
                                <span className="text-[9px] font-bold text-gray-400">{result.grade}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        {result.factors.map((f, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs">
                                ...
                                <span className="font-bold font-mono text-gray-500">{f.score}/100</span>
                            </div>
                        ))}
                    </div>
