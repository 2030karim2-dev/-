...
const SalesReturnsView: React.FC<SalesReturnsViewProps> = ({ searchTerm: propSearchTerm, onViewDetails }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 h-full space-y-3 animate-in fade-in duration-300 pt-2" ref={printRef}>
      {/* Stats Cards */}
      <ReturnsStatsHeader
            ...
            {processedReturns.length > 0 && (
              <div className="shrink-0 bg-red-50 dark:bg-red-900/20 px-4 py-2.5 border-t border-red-200 dark:border-red-800 flex justify-between items-center">
                <span className="font-bold text-xs text-red-800 dark:text-red-300">
                  إجمالي المرتجعات ({processedReturns.length} من {returns?.length || 0}):
                </span>
                <span className="font-bold text-base text-red-600 dark:text-red-400 font-mono" dir="ltr">
