...
const AuditLogView: React.FC = () => {
    ...
    const { data: log,} = useItemMovement(selectedId || null);
        ...
        <div className="flex flex-col gap-3 animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-slate-900 p-2 rounded-none border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-2">
                ...
                <select
                    ...
                    className="flex-1 bg-transparent text-[11px] font-bold outline-none dark:text-white"
                >
            ...
            <div className="flex flex-col gap-1.5 relative">
                {selectedId && log && log.length > 0 ? (
                                    ...
                                    <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-3 flex flex-col gap-2 group-hover:border-blue-500/30 transition-all">
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center gap-2">
                                                <span className={cn(
                                                    "text-[8px] font-bold px-1.5 py-0.5 uppercase tracking-tighter",
                                                    isIncoming ? "bg-emerald-500 text-white" : "bg-rose-500 text-white"
                                                )}>
                                                    {isIncoming ? 'وارد' : 'صادر'}
                                                </span>
                                                <h4 className="text-[10px] font-bold text-gray-800 dark:text-slate-100">{entry.reference_type}</h4>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between bg-gray-50 dark:bg-slate-950 p-2 border dark:border-slate-800">
                                            <div className="flex items-center gap-3">
                                                <div className="flex flex-col">
                                                    <span className="text-[7px] font-bold text-gray-400 uppercase">الكمية</span>
                                                    <span dir="ltr" className={cn("text-xs font-bold font-mono", entry.quantity > 0 ? "text-emerald-600" : "text-rose-600")}>
                                                        {entry.quantity > 0 ? '+' : ''}{formatNumberDisplay(entry.quantity)}
                                                    </span>
                                                </div>
                                                <div className="w-px h-6 bg-gray-200 dark:bg-slate-800"></div>
                                                <div className="flex flex-col">
                                                    <span className="text-[7px] font-bold text-gray-400 uppercase">الرصيد بعدها</span>
                                                    <span dir="ltr" className="text-xs font-bold font-mono text-blue-600">{formatNumberDisplay(entry.balance_after)}</span>
                                                </div>
                                            </div>
                    ...
                    <div className="p-20 text-center flex flex-col items-center gap-3 opacity-30">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-widest">بانتظار تحديد صنف للتدقيق</p>
                    </div>
                )}
            </div>
        </div>
