...
import { Receipt, Plus, PieChart as PieIcon, List } from 'lucide-react';
...
const ExpensesPage: React.FC = () => {
  ...
  const { expenses, isLoading, isError: _isError, stats } = useExpensesData(searchTerm);
      ...
      <MicroHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <button onClick={() => setIsModalOpen(true)} className="bg-rose-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold active:scale-95 flex items-center gap-1.5 shadow-lg shadow-rose-500/20">
        ...
        <div className="max-w-7xl mx-auto space-y-2">
          ...
          {viewMode === 'analytics' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 animate-in fade-in duration-500">
              <Card className="p-4">
                <h3 className="text-xs font-bold text-gray-700 dark:text-slate-300 mb-4">توزيع المصاريف</h3>
                <ExpenseBreakdownChart data={breakdownData} />
              </Card>
              <Card className="p-4">
                <h3 className="text-xs font-bold text-gray-700 dark:text-slate-300 mb-4">ملخص مالي</h3>
                <div className="space-y-2">
                  {breakdownData.map(item => (
                    <div key={item.name} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg">
                      ...
                      <span dir="ltr" className="text-[10px] font-bold font-mono">{formatCurrency(item.value)}</span>
                    </div>
                  ))}
                </div>
              </Card>
