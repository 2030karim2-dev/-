...
interface ExcelTableProps<T> {
  ...
  onCellUpdate?:
    | ((rowIndex: number, accessorKey: string, value: unknown) => void | Promise<void>)
    | undefined;
...
function ExcelTable<T>({
  ...
  const {
    ...
    zoomLevel: propZoomLevel !== undefined ? propZoomLevel : 100,
    setZoomLevel: propSetZoomLevel !== undefined ? propSetZoomLevel : () => {},
  });
  ...
  } = useTableDragDrop(paginatedResult as T[], onOrderChange);
    ...
    >
      <div
        className={cn(
          ...
          isZoomed ? 'h-full bg-white p-4 dark:bg-slate-950' : 'h-full'
        )}
      >
        <ExcelTableToolbar
          title={title}
        ...
        <div
          ref={tableWrapperRef}
          className="relative flex flex-1 flex-col overflow-hidden rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-sm"
          style={{
        ...
        >
          ...
          <div
            ref={tableRef}
            tabIndex={-1}
            className="data-table-scroll custom-scrollbar smooth-scroll flex-1 overflow-auto outline-none"
            onMouseDown={handleMouseDownOnTable}
            onMouseUp={() => {
              handleMouseUpOnTable(endSelection);
            }}
            onMouseLeave={() => {
              handleMouseLeaveOnTable(endSelection);
            }}
          >
            <table
              style={{ fontSize: `${zoomLevel * 11}px` }}
              className={cn(
                'w-full min-w-[800px] table-auto border-collapse border-l border-t border-[var(--app-border)]',
            ...
            >
              ...
              <ExcelTableBody
                ...
                handleMouseDownCell={(e: React.MouseEvent, row: number, col: number) => {
                  handleMouseDownCell(e, row, col, handleCellClick, startSelection);
                }}
