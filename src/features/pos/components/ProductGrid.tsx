    ...
    return (
        <div className="p-2 h-full overflow-y-auto pb-32 custom-scrollbar">
            <div className="flex gap-1.5 overflow-x-auto pb-3 mb-1.5 scrollbar-hide no-scrollbar">
                <button className="px-4 py-1.5 bg-blue-600 text-white rounded-lg font-bold text-[9px] whitespace-nowrap shadow-md">الكل</button>
                {['محركات', 'هيكل', 'زيوت', 'كهرباء', 'تكييف'].map(cat => (
                    <button key={cat} className="px-4 py-1.5 bg-white dark:bg-slate-900 text-gray-500 dark:text-slate-400 border border-gray-100 dark:border-slate-800 hover:bg-gray-50 rounded-lg font-bold text-[9px] whitespace-nowrap">{cat}</button>
                ))}
            </div>
                    ...
                    `}
                        >
                            <div className="flex-1">
                                <h3 className="font-bold text-gray-800 dark:text-slate-100 line-clamp-2 text-[10px] md:text-[11px] leading-tight">
                                    {product.name}
                                </h3>
                            ...
                            <div className="mt-1 flex justify-between items-center w-full">
                                <span dir="ltr" className="font-bold text-blue-600 dark:text-blue-400 text-[10px] md:text-xs">
                                    {formatCurrency(product.sale_price ?? product.selling_price ?? 0)}
                                </span>
                                <span dir="ltr" className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded ${hasStock ? 'text-gray-400 bg-gray-50 dark:bg-slate-800' : 'text-red-500 bg-red-100'}`}>
                            ...
                            {!hasStock && (
                                <div className="absolute inset-0 bg-white/20 backdrop-blur-[1px] rounded-xl flex items-center justify-center">
                                    <span className="bg-red-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full shadow-sm">نفذت</span>
                                </div>
                            )}
                        </button>
                    );
