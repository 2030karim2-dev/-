        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-4 border-b dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 className="text-xs font-bold text-gray-800 dark:text-white">محسّن الأرباح</h3>
                        <p className="text-[8px] text-gray-400 font-bold">تحليل حقيقي لفرص الربح</p>
                    </div>
                </div>
                <button onClick={optimize} disabled={isLoading}
                    className="px-3 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl text-[10px] font-bold hover:opacity-90 transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50 flex items-center gap-1"
                >
                ...
                {suggestions.length > 0 ? (
                    <>
                        <div className="flex items-center gap-1.5 mb-3 px-2 py-1 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg w-fit">
                            ...
                            <span className="text-[8px] font-bold text-emerald-600 dark:text-emerald-400">مصدر: بيانات النظام الحية</span>
                        ...
                        <div className="space-y-2">
                            {suggestions.map((tip, i) => (
                                <div key={i} className="flex items-start gap-2 p-2.5 bg-emerald-50/50 dark:bg-emerald-950/10 rounded-xl border border-emerald-100/30 dark:border-emerald-900/10">
                                    <span className="text-[10px] font-bold text-emerald-500 bg-emerald-100 dark:bg-emerald-900/30 w-5 h-5 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
