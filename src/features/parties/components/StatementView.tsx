        ...
        {selectedPartyId && selectedParty && (
          <div className="flex gap-2 mt-auto">
            <ShareButton
              ...
              message={`📄 كشف حساب: ${selectedParty.name}\n━━━━━━━━━━━━━━\n${(statement || []).map((row: StatementMovement) => `${row.date} | ${row.desc} | مدين: ${formatCurrency(row.debit)} | دائن: ${formatCurrency(row.credit)} | رصيد: ${formatCurrency(row.balance || 0)}`).join('\n')}\n━━━━━━━━━━━━━━\n📊 الرصيد النهائي: ${formatCurrency(statement?.length ? (statement[statement.length - 1].balance || 0) : 0)}`}
            />
            <Button
              onClick={async () => {
