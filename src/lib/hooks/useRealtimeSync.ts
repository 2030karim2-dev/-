...
export const useRealtimeSync = () => {
    ...
    useEffect(() => {
        ...
        console.log('🔌 Connecting to Supabase Realtime for auto-refresh...');
        ...
        const channel = supabase.channel('global-db-changes')
            .on(
                ...
                (payload) => {
                    ...
                    if (preset) {
                        console.log(`🔄 Realtime update on table [${table}], refreshing queries...`);
                        invalidateByPreset(queryClient, preset);
                    } else {
                        // Fallback for unmapped tables that might still need dashboard refresh
                        console.log(`🔄 Unmapped Realtime update on table [${table}], refreshing dashboard...`);
