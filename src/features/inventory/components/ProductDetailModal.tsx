...
const ProductDetailModal: React.FC<Props> = ({ product, onClose, onEdit }) => {
    ...
    const { data: analytics } = useQuery({
        ...
        queryFn: async () => {
            ...
            const sales = txs.filter((t: Record<string, unknown>) => t.transaction_type === 'out').reduce((sum: number, t: Record<string, unknown>) => sum + (t.quantity as number), 0);
            const purchases = txs.filter((t: Record<string, unknown>) => t.transaction_type === 'in').reduce((sum: number, t: Record<string, unknown>) => sum + (t.quantity as number), 0);
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100 leading-tight">{product.name}</h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-4 space-y-4">
                            ...
                            <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border dark:border-slate-800 shadow-sm">
                                <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-4 tracking-widest">بطاقة الصنف</h4>
                        ...
                        <div className="lg:col-span-8 flex flex-col gap-6">
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <StatCard icon={TrendingUp} label="سعر البيع" value={formatCurrency(product.sale_price ?? product.selling_price ?? 0)} color="text-blue-600" />
                                <StatCard icon={Activity} label="هامش الربح" value={`${margin.toFixed(1)}%`} color={margin > 0 ? "text-emerald-600" : "text-rose-600"} />
                            </div>

                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <StatCard icon={Package} label="المشتريات" value={formatNumberDisplay(stats.total_purchases as any)} color="text-indigo-600" />
                                <StatCard icon={TrendingDown} label="المبيعات" value={formatNumberDisplay(stats.total_sales as any)} color="text-purple-600" />
                                <StatCard icon={DollarSign} label="الأرباح" value={formatCurrency(stats.profit as any)} color="text-emerald-600" />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border dark:border-slate-800 shadow-sm flex flex-col">
                                    <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-4 flex items-center gap-2"><Layers size={14} /> التوزيع الجغرافي</h4>
                                        ...
                                        {product.warehouse_distribution?.map((wh, i) => (
                                            <div key={i} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-transparent hover:border-gray-200 dark:hover:border-slate-700 transition-colors">
                                                ...
                                                <div className="bg-white dark:bg-slate-800 border dark:border-slate-700 px-3 py-1.5 rounded-xl shadow-sm">
                                                    <span dir="ltr" className="text-sm font-bold text-blue-600 font-mono">{formatNumberDisplay(wh.quantity)}</span>
                                                </div>
                                            </div>
