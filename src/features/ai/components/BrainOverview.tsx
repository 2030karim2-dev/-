...
const BrainOverview: React.FC<Props> = ({ data,}) => {
    const kpis = [
                ...
                return (
                    <div key={i} className={`${kpi.bg} rounded-2xl p-3.5 border border-transparent hover:shadow-sm transition-all group`}>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[9px] font-bold text-gray-400 dark:text-slate-500 uppercase">{kpi.label}</span>
                        </div>
                        <p className={`text-sm font-bold font-mono ${kpi.text} group-hover:scale-105 transition-transform origin-right`}>{kpi.value}</p>
                    </div>
                );
            })}
        </div>
