        ...
        <div className="flex flex-col h-full bg-white dark:bg-slate-900 transition-colors duration-300">
            <div className="bg-gray-100 dark:bg-slate-950/50 border-b-2 border-blue-600 p-2 space-y-2">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-gray-800 dark:text-white uppercase tracking-widest">{t('active_cart')}</span>
                        <span className="bg-blue-600 text-white text-[9px] px-1.5 rounded-full font-bold min-w-[18px] text-center">{validItems.length}</span>
                    </div>
                    <button
                        onClick={onSuspend}
                        disabled={validItems.length === 0}
                        className="text-[9px] font-bold text-amber-600 hover:bg-amber-100 dark:hover:bg-amber-900/20 px-2 py-1 rounded transition-all flex items-center gap-1 uppercase disabled:opacity-30"
                    >
                ...
                <div className="flex gap-1">
                    ...
                    <div className="flex-1 relative">
                        ...
                        <select
                            ...
                            className="w-full h-[38px] bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-[9px] font-bold pr-6 pl-1 outline-none appearance-none focus:ring-2 focus:ring-blue-500/20 dark:text-white text-center cursor-pointer"
                        >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900">
                {validItems.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-gray-300 dark:text-slate-800 opacity-40">
                        ...
                        <p className="text-[10px] font-bold uppercase mt-3 tracking-[0.2em]">{t('waiting_for_items')}</p>
                    </div>
                ) : (
            ...
            <div className="p-3 bg-slate-900 text-white flex flex-col gap-3">
                ...
                <div className="flex justify-between items-center py-2 border-t border-white/10">
                    <span className="font-bold text-[10px] uppercase tracking-[0.2em] text-blue-400">{t('net_payable')}</span>
                    <div className="text-right">
                        <span dir="ltr" className="font-bold text-2xl text-white font-mono tracking-tighter leading-none block">
                            {formatCurrency(summary.totalAmount)}
                        </span>
                        <span className="text-[9px] font-bold text-blue-500 uppercase">{currency}</span>
                    </div>
                </div>
                ...
                <button
                    onClick={onPay}
                    disabled={validItems.length === 0}
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-800 disabled:text-slate-600 py-3.5 font-bold text-[11px] uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 active:scale-95 shadow-2xl group"
                >
