...
const CreateBondModal: React.FC<CreateBondModalProps> = ({ isOpen, onClose, type, onSubmit, isSubmitting }) => {
  ...
  const { data: customers } = useCustomerSearch(companyId, partyQuery);
  const { data: suppliers } = useSupplierSearch(companyId, partyQuery);
  ...
  const footer = (
    <div className="flex w-full gap-2 p-1">
      <button onClick={onClose} className="flex-1 py-3 text-[11px] font-bold text-gray-500 bg-gray-50 dark:bg-slate-800 border dark:border-slate-700 uppercase">إلغاء</button>
      <Button onClick={handleSubmit(onSubmit)} isLoading={isSubmitting} variant={type === 'receipt' ? 'success' : 'danger'}
        className="flex-[2] rounded-none text-[11px] font-bold shadow-xl uppercase" leftIcon={<Save size={16} />}>
        حفظ السند
      </Button>
    </div>
  );
  ...
  const CounterpartySelector = (
    <div className="space-y-2">
      <div className="flex h-10 bg-gray-100 dark:bg-slate-800 p-1 rounded-xl border dark:border-slate-700">
        <button type="button" onClick={() => setValue('counterparty_type', 'party')} className={cn("flex-1 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1", counterpartyType === 'party' ? "bg-white dark:bg-slate-600 text-blue-600 shadow-sm" : "text-gray-400")}><Building size={12} /> جهة خارجية</button>
        <button type="button" onClick={() => setValue('counterparty_type', 'account')} className={cn("flex-1 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1", counterpartyType === 'account' ? "bg-white dark:bg-slate-600 text-blue-600 shadow-sm" : "text-gray-400")}><Landmark size={12} /> حساب عام</button>
      </div>
            ...
            <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 rounded-xl shadow-lg max-h-40 overflow-auto border dark:border-slate-600 animate-in fade-in slide-in-from-top-2">
              {parties.map((p: any) => (
                <div key={p.id} onClick={() => handlePartySelect(p)} className="p-2.5 hover:bg-blue-50 dark:hover:bg-slate-600 cursor-pointer flex justify-between items-center">
                  ...
                  <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${p.type === 'customer' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{p.type === 'customer' ? 'عميل' : 'مورد'}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <form className="flex flex-col">
        <div className={`p-5 bg-white dark:bg-slate-900 border-b dark:border-slate-800 flex items-end gap-4`}>
          <div className="flex-1">
            <label className={`block text-[10px] font-bold text-${theme.color}-600 dark:text-${theme.color}-400 uppercase tracking-widest mb-1.5 px-1`}>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 bg-white dark:bg-slate-950 border-2 border-${theme.color}-100 dark:border-${theme.color}-900/30 rounded-xl text-2xl font-bold text-${theme.color}-600 dark:text-${theme.color}-400 outline-none focus:border-${theme.color}-500 font-mono`}
                placeholder="0.00"
          ...
          {selectedCurrency !== 'SAR' && (
            <div className="w-32 shrink-0">
              <label className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 px-1">المقابل (SAR)</label>
              <input
                type="number"
                step="0.01"
                {...register('amount', { required: true, valueAsNumber: true })}
                readOnly
                className="w-full px-3 py-3 bg-gray-50 dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-800 rounded-xl text-sm font-bold text-gray-500 font-mono text-center outline-none"
                tabIndex={-1}
              />
            </div>
          )}
          ...
          <div className="w-24 shrink-0 space-y-1.5">
            <label className="text-[9px] font-bold text-gray-400 uppercase text-center">العملة</label>
            <select {...register('currency_code')} className="w-full bg-gray-50 dark:bg-slate-800 border-2 dark:border-slate-700 p-3 rounded-xl text-sm font-bold outline-none appearance-none text-center">
        ...
        <div className="p-5 border-b dark:border-slate-800 bg-gray-50/30 dark:bg-slate-950/30 space-y-3">
          <div className={`p-4 rounded-2xl border-2 bg-white dark:bg-slate-900 border-gray-100 dark:border-slate-800 space-y-3`}>
            <h3 className={`text-xs font-bold text-${theme.color}-600`}>{theme.verbFrom}</h3>
          ...
          <div className={`p-4 rounded-2xl border-2 bg-white dark:bg-slate-900 border-gray-100 dark:border-slate-800 space-y-3`}>
            <h3 className="text-xs font-bold text-gray-500">{theme.verbTo}</h3>
