...
const ProductExcelGrid: React.FC<Props> = ({ products, isLoading, onDelete, onViewDetails, onEdit }) => {
  ...
  const [deleteConfirm, setDeleteConfirm] = React.useState<{ id: string | string[]; type: 'single' | 'bulk' } | null>(null);

  const handleOrderChange = (_reorderedProducts: Product[]) => {
      ...
      accessor: (p) => (
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <span className="font-bold text-sm text-slate-900 dark:text-white block truncate">{p.name}</span>
    ...
    },
    {
      ...
      accessor: (p) => (
        <div className="flex justify-center items-center gap-1.5">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ id: p.id, type: 'single' }); }}
