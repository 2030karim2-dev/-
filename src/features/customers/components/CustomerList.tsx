...
const CustomerList: React.FC<Props> = ({ customers, onEdit, onDelete }) => {
  const columns = [
    { 
         ...
         */
        accessor: (row: Customer) => <span className="font-bold text-gray-800 dark:text-slate-100">{row.name}</span> 
    },
    { header: 'رقم الهاتف', accessor: (row: Customer) => <span dir="ltr" className="font-mono text-gray-500">{row.phone || 'N/A'}</span>, width: "w-40" },
    { header: 'الفئة', accessor: (row: Customer) => <span className="text-xs font-bold bg-gray-50 dark:bg-slate-800 px-2 py-1 border dark:border-slate-700">{row.category || 'عام'}</span>, width: "w-32", className: "text-center" },
    { header: 'الحالة', accessor: (row: Customer) => (
    ...
    { header: 'الرصيد', accessor: (row: Customer) => (
    ...
    { header: 'إجراءات', accessor: (row: Customer) => (
  ...
  return <ExcelTable columns={columns} data={customers} colorTheme="green" />;
};
