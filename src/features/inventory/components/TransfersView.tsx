...
const TransfersView: React.FC = () => {
  ...
  const smartSuggestions = useMemo<SmartSuggestion[]>(() => {
    ...
    (warehouses as Record<string, unknown>[]).forEach((w) => whMap.set(w.id as string, (w.name || w.name_ar) as string));
  ...
  const stats = useMemo(() => ({
    ...
    thisMonth: (transfers || []).filter((t: Record<string, unknown>) => {
      const d = new Date(t.created_at as string);
      const now = new Date();
      return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
    }).length,
    totalItems: (transfers || []).reduce((s: number, t: Record<string, unknown>) => s + (Number(t.item_count) || 0), 0),
  ...
  const columns = [
    { header: '#', accessor: (row: Record<string, unknown>) => <span className="font-mono font-bold text-gray-400">{(row.transfer_number as string) || '-'}</span>, width: 'w-24' },
    {
      header: 'من مستودع', accessor: (row: Record<string, unknown>) => (
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-rose-500"></div>
          <span className="font-bold text-rose-600 dark:text-rose-400">{row.from_warehouse_name as string}</span>
        </div>
      ), sortKey: 'from_warehouse_name'
    },
    {
      header: 'إلى مستودع', accessor: (row: Record<string, unknown>) => (
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
          <span className="font-bold text-emerald-600 dark:text-emerald-400">{row.to_warehouse_name as string}</span>
        </div>
      ), sortKey: 'to_warehouse_name'
    },
    {
      header: 'التاريخ', accessor: (row: Record<string, unknown>) => (
        <span className="text-gray-600 dark:text-gray-400 font-mono text-[10px]">
          {new Date(row.created_at as string).toLocaleDateString('ar-SA', { year: 'numeric', month: 'short', day: 'numeric' })}
        </span>
      ), width: 'w-32', sortKey: 'created_at'
    },
    {
      header: 'الأصناف', accessor: (row: Record<string, unknown>) => (
        <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded-full text-[10px] font-bold">
          {row.item_count as number} أصناف
        </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 p-3 flex items-center gap-3 shadow-sm">
            ...
            <div>
              <span className="text-[9px] font-bold text-gray-400 uppercase block">{s.label}</span>
              <span className="text-lg font-bold font-mono text-gray-800 dark:text-gray-100">{s.value}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="bg-gradient-to-l from-violet-50 to-indigo-50 dark:from-violet-950/30 dark:to-indigo-950/30 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 p-4 space-y-3 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100">اقتراحات ذكية للمناقلة</h3>
                <p className="text-[10px] text-gray-500 font-bold">بناءً على تحليل توزيع المخزون بين المستودعات</p>
              </div>
            </div>
            <span className="text-[9px] font-bold bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400 px-2 py-1 rounded-full">
              ...
              <div key={i} className={cn("rounded-xl border p-3 transition-all hover:shadow-md", priorityColors[s.priority])}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-gray-800 dark:text-gray-100 truncate">{s.productName}</p>
                    <span className={cn("text-[8px] font-bold px-1.5 py-0.5 rounded mt-1 inline-block", priorityBadges[s.priority])}>
                      {s.reason}
                    </span>
                  </div>
                  <span className="text-lg font-bold font-mono text-indigo-600 dark:text-indigo-400 shrink-0">
                    {s.suggestedQty}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="flex-1 text-center py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-lg transition-colors shadow-sm"
                  >
                    تنفيذ المناقلة
                  </button>
                  <button
