...
export const useProductMutations = () => {
    ...
    const saveProduct = useMutation({
        ...
        onSuccess: (savedProduct, variables) => {
            ...
            if (savedProduct) {
                queryClient.setQueryData(['products', user?.company_id], (oldData: Product[] | undefined) => {
        ...
        onError: (error: Error) => showToast("فشل الحفظ: " + error.message, 'error')
    });
    ...
    const deleteProduct = useMutation({
        ...
        onError: (error: Error) => showToast(error.message || "فشل حذف المنتج", 'error')
    });
    ...
    const bulkDeleteProducts = useMutation({
        ...
        onError: (error: Error) => showToast(error.message || "فشل حذف المنتجات", 'error')
    });
...
export const useInventoryMutations = () => {
    ...
    const transfer = useMutation({
        mutationFn: (data: { from_warehouse_id: string; to_warehouse_id: string; items: { product_id: string; quantity: number }[]; notes?: string }) => {
