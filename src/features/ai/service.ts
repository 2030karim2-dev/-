...
import { FinancialDataSnapshot, AIInsight, InventoryDataSnapshot, InventoryAIInsight, SmartPricingResult, SalesForecastResult, SmartPurchaseOrdersResult, InvoiceSuspicionResult, StockDepletionResult, CustomerSegmentationResult, CrossSellResult, SupplierRatingResult, InvoiceCommandResult, JournalEntrySuggestion, BusinessHealthResult, AnomalyDetectionResult, MarketPositionResult } from './types';
...
export const aiService = {
  ...
  generateInventoryAnalysis: async (data: InventoryDataSnapshot): Promise<InventoryAIInsight> => {
    ...
    try {
      ...
      logger.error('AI', 'Inventory analysis failed', e as Error);
  ...
  generateDailySummary: async (context: string): Promise<string> => {
    ...
    try {
      ...
      logger.error('AI', 'Daily summary failed', e as Error);
   ...
   */
  generateSmartPricing: async (product: { name: string; purchasePrice: number; currentPrice: number; avgSales: number }): Promise<SmartPricingResult> => {
...
{"suggestedPrice": number, "reason": "سبب مختصر بالعربي"}`;
    ...
    try {
      ...
      return JSON.parse(cleaned) as SmartPricingResult;
    } catch (e) {
      logger.error('AI', 'Smart pricing failed', e as Error);
   ...
   * توقع المبيعات
   */
  generateSalesForecast: async (monthlySales: { month: string; total: number }[]): Promise<SalesForecastResult> => {
...
{"forecast": number, "trend": "صاعد|هابط|مستقر", "tips": ["نصيحة1", "نصيحة2"]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      const cleaned = result.replace(/```json|```/g, '').trim();
      return JSON.parse(cleaned) as SalesForecastResult;
    } catch (e) {
      logger.error('AI', 'Sales forecast failed', e as Error);
   ...
   */
  generateSmartPurchaseOrders: async (lowStockItems: { name: string; current: number; minStock: number; avgMonthlyUsage: number }[]): Promise<SmartPurchaseOrdersResult> => {
...
{"items": [{"name": "اسم", "suggestedQty": number, "priority": "عاجل|متوسط|منخفض"}], "summary": "ملخص مختصر"}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      const cleaned = result.replace(/```json|```/g, '').trim();
      return JSON.parse(cleaned) as SmartPurchaseOrdersResult;
    } catch (e) {
      logger.error('AI', 'Smart purchase orders failed', e as Error);
   ...
   */
  analyzeInvoiceSuspicion: async (invoice: { number: string; total: number; itemCount: number; customerName: string; customerDebt: number; avgInvoiceTotal: number }): Promise<InvoiceSuspicionResult> => {
...
{"riskLevel": "low|medium|high", "alerts": ["تنبيه1..."]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      const cleaned = result.replace(/```json|```/g, '').trim();
      return JSON.parse(cleaned) as InvoiceSuspicionResult;
    } catch (e) {
      logger.error('AI', 'Invoice suspicion analysis failed', e as Error);
   ...
   */
  predictStockDepletion: async (products: { name: string; currentStock: number; avgDailySales: number }[]): Promise<StockDepletionResult> => {
...
{"items": [{"name": "اسم", "daysLeft": number, "urgency": "حرج|تحذير|آمن"}]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as StockDepletionResult;
    } catch (e) {
      logger.error('AI', 'Stock depletion prediction failed', e as Error);
   ...
   * تصنيف العملاء الذكي
   */
  segmentCustomers: async (customers: { name: string; totalPurchases: number; lastPurchaseDate: string; invoiceCount: number; balance: number }[]): Promise<CustomerSegmentationResult> => {
...
{"segments": [{"name": "اسم", "segment": "VIP|معرض للانسحاب|جديد|عادي", "recommendation": "توصية مختصرة"}]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as CustomerSegmentationResult;
    } catch (e) {
      logger.error('AI', 'Customer segmentation failed', e as Error);
      return { segments: [] };
    }
  },
   ...
   */
  suggestCrossSell: async (currentItems: string[]): Promise<CrossSellResult> => {
...
{"suggestions": [{"name": "اسم المنتج", "reason": "سبب مختصر"}]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as CrossSellResult;
    } catch (e) {
      logger.error('AI', 'Cross-sell suggestion failed', e as Error);
      return { suggestions: [] };
    }
  },
   ...
   */
  rateSuppliers: async (suppliers: { name: string; totalPurchases: number; avgDeliveryDays: number; returnRate: number; balance: number }[]): Promise<SupplierRatingResult> => {
...
{"ratings": [{"name": "اسم", "score": number, "strengths": "نقاط قوة", "weaknesses": "نقاط ضعف"}]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as SupplierRatingResult;
    } catch (e) {
      logger.error('AI', 'Supplier rating failed', e as Error);
      return { ratings: [] };
    }
  },
   ...
   */
  parseInvoiceCommand: async (command: string): Promise<InvoiceCommandResult> => {
...
{"customerName": "اسم العميل أو فارغ", "items": [{"name": "اسم المنتج", "quantity": number, "price": number}], "paymentMethod": "cash|credit"}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as InvoiceCommandResult;
    } catch (e) {
      logger.error('AI', 'Invoice command parsing failed', e as Error);
...
أنشئ تقريراً مختصراً وعملياً بالعربية مع أرقام وإحصائيات. استخدم إيموجي باعتدال. لا JSON.`;
    ...
    try {
      ...
      logger.error('AI', 'Custom report generation failed', e as Error);
   ...
   */
  suggestJournalEntry: async (description: string, amount: number): Promise<JournalEntrySuggestion> => {
...
{"debitAccount": "الحساب المدين", "creditAccount": "الحساب الدائن", "explanation": "شرح مختصر"}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as JournalEntrySuggestion;
    } catch (e) {
      logger.error('AI', 'Journal entry suggestion failed', e as Error);
...
- لا JSON — نص عادي فقط`;
    ...
    try {
      ...
      logger.error('AI', 'Morning brief generation failed', e as Error);
   ...
   */
  calculateBusinessHealth: async (data: { revenue: number; expenses: number; netProfit: number; receivables: number; payables: number; liquidity: number; lowStockCount: number; totalProducts: number }): Promise<BusinessHealthResult> => {
...
{"score": number, "grade": "ممتاز|جيد جداً|جيد|مقبول|ضعيف", "factors": [{"name": "اسم العامل", "impact": "إيجابي|سلبي", "score": number}], "advice": "نصيحة واحدة أهم"}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as BusinessHealthResult;
    } catch (e) {
      logger.error('AI', 'Business health calculation failed', e as Error);
   ...
   */
  detectAnomalies: async (transactions: { type: string; amount: number; date: string; description: string }[]): Promise<AnomalyDetectionResult> => {
    const prompt = `أنت مدقق مالي خبير في الجعفري لقطع غيار السيارات.
...
{"anomalies": [{"description": "وصف", "amount": number, "reason": "سبب الشبهة", "severity": "عالي|متوسط|منخفض"}]}`;
    ...
    try {
      const result = await aiApi.analyzeFinancials(prompt);
      return JSON.parse(result.replace(/```json|```/g, '').trim()) as AnomalyDetectionResult;
    } catch (e) {
      logger.error('AI', 'Anomaly detection failed', e as Error);
      return { anomalies: [] };
    }
  },
   ...
   */
  analyzeMarketPosition: async (data: { topProducts: string[]; avgMargin: number; monthlyRevenue: number }): Promise<MarketPositionResult> => {
    const prompt = `أنت خبير سوق قطع غيار سيارات في المنطقة العربية.
...
{"insights": ["ملاحظة1", "ملاحظة2"], "opportunities": ["فرصة1"], "threats": ["تهديد1"]}`;
    ...
    try {
      ...
      return JSON.parse(cleaned) as MarketPositionResult;
    } catch (e) {
      logger.error('AI', 'Market position analysis failed', e as Error);
