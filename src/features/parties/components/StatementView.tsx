...
const StatementView: React.FC<{ partyType: PartyType }> = ({ partyType }) => {
  ...
  const columns = [
    ...
    { header: 'مدين', accessor: (row: any) => row.debit > 0 ? <span dir="ltr">{formatCurrency(row.debit)}</span> : '-', className: 'text-left font-mono text-emerald-700' },
    { header: 'دائن', accessor: (row: any) => row.credit > 0 ? <span dir="ltr">{formatCurrency(row.credit)}</span> : '-', className: 'text-left font-mono text-rose-700' },
        ...
        {selectedPartyId && selectedParty && (
          <div className="flex gap-2 mt-auto">
            <ShareButton
              ...
              message={`📄 كشف حساب: ${selectedParty.name}\n━━━━━━━━━━━━━━\n${((statement || []) as any[]).map((row: any) => `${row.date} | ${row.desc} | مدين: ${formatCurrency(row.debit)} | دائن: ${formatCurrency(row.credit)} | رصيد: ${formatCurrency(row.balance)}`).join('\n')}\n━━━━━━━━━━━━━━\n📊 الرصيد النهائي: ${formatCurrency(((statement || []) as any[]).at(-1)?.balance || 0)}`}
            />
